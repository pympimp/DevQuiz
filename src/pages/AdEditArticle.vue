...
const fetchOneCourses = async() =>{
  const result = await axios.get(` http://172.16.49.36:3000/coures/${coursesId.value}/${lessonId.value}/${unitId.value}`)
  if(result){
...
await axios.put(`http://172.16.49.36:3000/coures/editCourse/${coursesId.value}/${lessonId.value}/${unitId.value}`,InputData.value)
}
const goback = () => {
  router.go(-1);
}

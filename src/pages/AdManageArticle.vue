...
const deleteUnit = async (coursesId, lessonId, unitId) => {
  const result = await axios.delete(
    ` http://172.16.49.36:3000/coures/deleteCourse/${coursesId}/${lessonId}/${unitId}`
  )
  if (result) {
    fetchOneCourses()
  }
...
const addLesson = async (finalName) => {
  const result = await axios.post(
    `http://172.16.49.36:3000/coures/addLesson/${data.value.CoursesId}`,
    {
...
const deleteLesson = async (coursesId, lessonId) => {
  const result = await axios.delete(
    `http://172.16.49.36:3000/coures/deleteLesson/${coursesId}/${lessonId}`
  )
  if (result) {
    fetchOneCourses()
  }

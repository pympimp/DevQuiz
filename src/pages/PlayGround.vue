...
const fetchOneUnit = async (couresId, lessonId, unitId) => {
  const result = await axios.get(
    `https://functions-khaki-gamma.vercel.app/coures/${couresId}/${lessonId}/${unitId}`
  )
  if (result) {
...
const check = async () => {
  ...
  try {
    const result = await axios.put(
      `https://functions-khaki-gamma.vercel.app/user/Test/${authenStore.auth.id}/${course.value}`,
      {
        [lesson]: {
          [units]: true
        }
